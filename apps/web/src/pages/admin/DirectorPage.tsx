    ...
    <section className="rounded-2xl bg-white p-5 shadow-sm">
      ...
      {prepared && catalog && <form className="mt-4 space-y-4 border-t pt-4" onSubmit={(e) => { e.preventDefault(); void send() }}>
        ...
        <fieldset disabled={saving} className="grid gap-3 sm:grid-cols-2">
          <label className="text-sm">Участок<select aria-label="Участок" required className={fieldClass} value={draft.sectionId} onChange={(e) => setDraft({ ...draft, sectionId: e.target.value, assigneeUserId: '', assignmentReason: '' })}><option value="">Уточните участок</option>{catalog.sections.map((s) => <option key={s.id} value={s.id}>{s.objectName} / {s.name} ({s.code})</option>)}</select></label>
          <label className="text-sm">Вид работы<select aria-label="Вид работы" required className={fieldClass} value={draft.workTypeId} onChange={(e) => setDraft({ ...draft, workTypeId: e.target.value, assigneeUserId: '', assignmentReason: '' })}><option value="">Уточните вид работы</option>{catalog.workTypes.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select></label>
          <label className="text-sm">Исполнитель<select aria-label="Исполнитель" required className={fieldClass} value={draft.assigneeUserId} onChange={(e) => setDraft({ ...draft, assigneeUserId: e.target.value, assignmentReason: 'Исполнитель выбран вручную.' })}><option value="">Уточните исполнителя</option>{catalog.assignees.map((u) => <option key={u.id} value={u.id}>{u.fullName}</option>)}</select></label>
