...
import { fetchActiveVehicleTypes, fetchAllVehicleTypes, type VehicleTypeRef } from '@/api/vehicleTypesApi'
...
export function VehiclesPage() {
  ...
  const [activeTypes, setActiveTypes] = useState<VehicleTypeRef[]>([])
  const [allTypes, setAllTypes] = useState<VehicleTypeRef[]>([])
  ...
  async function load() {
    const [vehicleRows, brigadeRows, routeRows, activeTypeRows, allTypeRows] = await Promise.all([
      fetchVehicles(), fetchBrigades(), fetchRoutes(), fetchActiveVehicleTypes(), fetchAllVehicleTypes(),
    ])
    setVehicles(vehicleRows); setBrigades(brigadeRows); setRoutes(routeRows)
    setActiveTypes(activeTypeRows); setAllTypes(allTypeRows)
    setType((prev) => prev || activeTypeRows[0]?.key || '')
  }
  ...
  const typeLabel = (key: string) => allTypes.find((t) => t.key === key)?.name ?? key
