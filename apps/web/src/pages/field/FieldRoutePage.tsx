...
export function FieldRoutePage() {
  ...
  const statusLabel = route.status === 'IN_PROGRESS' ? 'В пути' : route.status === 'COMPLETED' ? 'Завершён' : route.status === 'CANCELLED' ? 'Отменён' : 'Запланирован'
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between"><div><h1 className="text-2xl font-black">Маршрут</h1><p className="text-sm text-slate-500">{route.brigade.name} · {route.stops.length} точек</p></div><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">{statusLabel}</span></div>
