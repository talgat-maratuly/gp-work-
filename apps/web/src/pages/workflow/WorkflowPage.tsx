...
function StandardForm({catalog,onChanged}:{catalog:Catalog;onChanged:()=>Promise<void>}) {
  const [busy,setBusy]=useState(false);const [error,setError]=useState('')
  async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const form=e.currentTarget;const d=new FormData(form);const lines=(key:string)=>String(d.get(key)).split('\n').map(v=>v.trim()).filter(Boolean);setBusy(true);setError('');try{await workflowPost('/standards',{workTypeId:Number(d.get('workTypeId')),title:d.get('title'),preparation:lines('preparation'),steps:lines('steps'),acceptance:d.get('acceptance')});form.reset();await onChanged()}catch(err){setError(toUserMessage(err))}finally{setBusy(false)}}
  return <details className={panelClass}><summary className="cursor-pointer font-bold">Создать стандарт или новую редакцию</summary><form onSubmit={submit} className="space-y-3"><fieldset disabled={busy} className="space-y-3"><Label name="Вид работы"><select name="workTypeId" className={inputClass} required defaultValue=""><option value="">Выберите вид работы</option>{catalog.workTypes.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></Label><Label name="Название стандарта"><input name="title" className={inputClass} required maxLength={160}/></Label><Label name="Подготовка — один пункт на строку"><textarea name="preparation" className={inputClass} rows={3} required/></Label><Label name="Шаги работы — один пункт на строку"><textarea name="steps" className={inputClass} rows={4} required/></Label><Label name="Критерии приёмки"><textarea name="acceptance" className={inputClass} rows={3} required maxLength={2000}/></Label><button className={buttonClass} type="submit">Сохранить новую версию</button></fieldset>{error&&<p role="alert" className="text-red-700">{error}</p>}</form></details>
}
