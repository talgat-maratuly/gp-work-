...
test('admin builds fields and stages; a worker fills a task process and a director approves it', async ({ page, context, request }, info) => {
  ...
  await expect(page.getByRole('region', { name: 'Как пользоваться бизнес-процессами', exact: true })).toContainText('Администратор или директор')
  ...
  await template.getByRole('link', { name: 'Подключить к задаче', exact: true }).click()
  await expect(page.getByRole('region', { name: 'Подключение процесса', exact: true })).toContainText(title)
  await page.getByRole('link').filter({ has: page.getByRole('heading', { name: `#${task.id} · ${task.description}`, exact: true }) }).click()
  ...
  await expect(processField).toHaveValue(String(definition.id))
  expect(await (await request.get(`${api}/business-processes/tasks/${task.id}`, { headers })).json()).toEqual([])
